...
public class SEServer {
    /**
     * Interface for the search service that processes queries.
     */
    public interface SearchService {
        /**
         * Processes a query, returning result as a string. The result may
         * contain HTML markup. It will be pasted as the content of the "body
         * element" in the search result page.
         */
     ...
     * @param service The service to pass queries to, to get search results.
     */
    public SEServer(int port, SearchService service) {
        this.port = port;
        this.service = service;
    }
    
    /**
     * Runs the server forever. Queries received via HTTP are passed on to the
     * search service, and the serch result returned to the client.
     */
    ...
    private final Pattern searchUrl = Pattern.compile("search\\?query=(.*)");
    ...
    private String processRequest(BufferedReader r) throws IOException {
        ...
        if ("".equals(url) || "index.html".equals(url)) {
            return "<html xmlns=\"http://www.w3.org/1999/xhtml\">" +
                ...
                "<title>Query Form</title></head><body>" +
                "<form method=\"get\" action=\"search\" accept-charset=\"UTF-8\">" +
                "<fieldset><label>Enter query</label><br /><input type=\"text\" name=\"query\" /><br /></fieldset>" +
                    "</form></body></html>";
        }
        Matcher urlM = searchUrl.matcher(url);
        if (!urlM.matches()) {
            throw new HttpStatusException(STATUS_NOT_FOUND, "Page not found on server: " + url);
        }
        return service.search(translateUrl(new StringReader(urlM.group(1))));
    }

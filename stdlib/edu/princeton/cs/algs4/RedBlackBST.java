 ...
 * 
 * A symbol table implemented using a left-leaning red-black tree. This is the
 * 2-3 version.
 * 
 * Reference: Algorithms in Java, 4th edition, Section 4.3.
 * 
 * % more tiny.txt S E A R C H E X A M P L E
 * 
 * % java RedBlackBST < tiny.txt A 8 C 4 E 12 H 5 L 11 M 9 P 10 R 3 S 0 X 7
 * 
 *************************************************************************/
...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
	...
	private boolean isRedBlackBST() {
		if (!isBST())
			StdOut.println("Not a BST");
		if (!is23())
			StdOut.println("Not a 23");
		if (!isBalanced())
	...
	private boolean isBST() {
		...
		if (!hasNoDuplicates())
			StdOut.println("Has duplicate keys");
		if (!isRankConsistent())
			StdOut.println("Rank not consistent");
		return isBinaryTree() && isOrdered() && hasNoDuplicates()
				&& isRankConsistent();
	}
	...
	// are all the values in the BST rooted at x between min and max, and
	// recursively?
	...
	private boolean hasNoDuplicates() {
		...
		for (int i = 1; i < size(); i++) {
			if (select(i).compareTo(select(i - 1)) == 0)
				return false;
		}
		return true;
	}
